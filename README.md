...
1. **Tushare Token** — 填入你的 Tushare Pro Token，用于获取行情数据, 具体查看 [Tushare 官网](https://tushare.pro/weborder/#/permission/buy))
   1. 基础使用需要 2000 积分：个股历史数据，指数历史数据
   2. 实时行情需要单独开通权限
   3. 情绪模块需要 6000 积分：涨停/跌停，连板梯队，板块轮动，成分股热力图
...
- [x] **涨停板复盘**
  - [x] 每日涨停股自动归类（首板 / 二板 / 三板 / 高位板）
  - [ ] 涨停时间线排序（早板 > 午板 > 尾板）
  - [ ] 封单金额 / 开板次数 / 换手率统计
  - [x] 炸板股追踪与分析（炸板率、亏钱效应图）

- [x] **连板梯队监控**
  - [x] 实时连板梯队展示（最高板 → 首板，梯形结构）
  - [x] 晋级率趋势图（1 进 2、2 进 3、高位晋级率）
  - [ ] 连板股趋势图（连板天数 vs 涨幅曲线）
  - [ ] 断板预警（高位股跌停 / 大幅低开）
  - [ ] 历史连板高度统计（市场连板天花板）

- [x] **题材热点追踪**
  - [x] 概念板块强度排名（每日涨幅 Top5、5/10/20 日累计涨幅）
  - [x] 成分股热力图（点击板块弹出正方形弹窗，Squarified 布局）
  - [ ] 涨停股按概念 / 行业自动聚类
